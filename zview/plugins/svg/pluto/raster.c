...
static void plutovg_span_buffer_update_extents(plutovg_span_buffer_t *span_buffer)
{
	...
	y1 = spans[0].y;
	x2 = 0;
	y2 = spans[span_buffer->spans.size - 1].y;

	for (i = 0; i < span_buffer->spans.size; i++)
	{
		if (spans[i].x < x1)
			x1 = spans[i].x;
...
void plutovg_span_buffer_intersect(plutovg_span_buffer_t *span_buffer, const plutovg_span_buffer_t *a,
	const plutovg_span_buffer_t *b)
{
	plutovg_span_t *a_spans;
	plutovg_span_t *a_end;
	plutovg_span_t *b_spans;
	plutovg_span_t *b_end;

	plutovg_span_buffer_reset(span_buffer);
	plutovg_array_ensure(span_buffer->spans, plutovg_max(a->spans.size, b->spans.size));

	a_spans = a->spans.data;
	a_end = a_spans + a->spans.size;

	b_spans = b->spans.data;
	b_end = b_spans + b->spans.size;

	while (a_spans < a_end && b_spans < b_end)
	{
		plutovg_int_t x, len;
		plutovg_int_t ax1, ax2, bx1, bx2;
		...
		x = plutovg_max(ax1, bx1);
		len = plutovg_min(ax2, bx2) - x;
		...
		if (len)
		{
			...
			span = span_buffer->spans.data + span_buffer->spans.size;
...
static PVG_FT_Outline *ft_outline_create(plutovg_int_t points, plutovg_int_t contours)
{
	PVG_FT_Outline *outline;
	PVG_FT_Byte *outline_data;
	size_t points_size = ALIGN_SIZE((points + contours) * sizeof(*outline->points));
	size_t tags_size = ALIGN_SIZE((points + contours) * sizeof(*outline->tags));
	size_t contours_size = ALIGN_SIZE(contours * sizeof(*outline->contours));
	size_t contours_flag_size = ALIGN_SIZE(contours * sizeof(*outline->contours_flag));

	outline = malloc(points_size + tags_size + contours_size + contours_flag_size + sizeof(PVG_FT_Outline));
	outline_data = (PVG_FT_Byte *) (outline + 1);

	outline->points = (PVG_FT_Vector *) (outline_data);
	outline->tags = (char *) (outline_data + points_size);
	outline->contours = (PVG_FT_Int *) (outline_data + points_size + tags_size);
...
static void ft_outline_close(PVG_FT_Outline *ft)
{
	...
	index = ft->n_contours ? ft->contours[ft->n_contours - 1] + 1 : 0;
...
static PVG_FT_Outline *ft_outline_convert_stroke(const plutovg_path_t * path, const plutovg_matrix_t * matrix,
	const plutovg_stroke_data_t * stroke_data);
	...
	const plutovg_stroke_data_t *stroke_data)
{
	...
	outline = ft_outline_create(path->num_points, path->num_contours);
...
static PVG_FT_Outline *ft_outline_convert_dash(const plutovg_path_t *path, const plutovg_matrix_t *matrix,
	const plutovg_stroke_dash_t *stroke_dash)
{
	plutovg_path_t *dashed;
	PVG_FT_Outline *outline;
	
	if (stroke_dash->array.size == 0)
		return ft_outline_convert(path, matrix, NULL);
	dashed = plutovg_path_clone_dashed(path, stroke_dash->offset, stroke_dash->array.data, stroke_dash->array.size);
	outline = ft_outline_convert(dashed, matrix, NULL);
	...
	const plutovg_stroke_data_t *stroke_data)
{
	...
	PVG_FT_Fixed ftMiterLimit = (PVG_FT_Fixed) (stroke_data->style.miter_limit * ((int32_t)1 << 16));
	...
	outline = ft_outline_convert_dash(path, matrix, &stroke_data->dash);

	PVG_FT_Stroker_ParseOutline(stroker, outline);

	PVG_FT_Stroker_GetCounts(stroker, &points, &contours);

	stroke_outline = ft_outline_create(points, contours);
...
static void spans_generation_callback(PVG_FT_Int count, const PVG_FT_Span *spans, void *user)
{
...
void plutovg_rasterize(plutovg_span_buffer_t *span_buffer, const plutovg_path_t *path, const plutovg_matrix_t *matrix,
	const plutovg_rect_t *clip_rect, const plutovg_stroke_data_t *stroke_data,
	plutovg_fill_rule_t winding)
{
	...
	outline = ft_outline_convert(path, matrix, stroke_data);
