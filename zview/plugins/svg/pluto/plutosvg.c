...
plutovg_int_t plutosvg_version(void)
{
...
static int lookupid(const char *data, size_t length, const name_entry_t *table, size_t count)
{
	...
	entry = bsearch(name, table, count / sizeof(name_entry_t), sizeof(name_entry_t), name_entry_compare);
...
static void *heap_alloc(heap_t *heap, size_t size)
{
	...
	data = (char *) (heap->chunk) + sizeof(heap_chunk_t) + heap->size;
...
static const string_t *find_attribute(const element_t *element, int id, bool inherit)
{
	do
	{
...
static bool has_attribute(const element_t *element, int id)
{
	const attribute_t *attribute = element->attributes;
...
static bool parse_float(const char **begin, const char *end, float *number)
{
	const char *it = *begin;
	float fraction = 0;
	float integer = 0;
...
static bool skip_string(const char **begin, const char *end, const char *data)
{
	const char *it = *begin;
...
static bool skip_delim(const char **begin, const char *end, const char delim)
{
	const char *it = *begin;
...
static bool skip_ws(const char **begin, const char *end)
{
	const char *it = *begin;
...
static bool skip_ws_delim(const char **begin, const char *end, char delim)
{
	const char *it = *begin;
...
static bool skip_ws_comma(const char **begin, const char *end)
{
...
static float convert_length(const length_t *length, float maximum)
{
...
static bool parse_color_value(const char **begin, const char *end, color_t *color)
{
	...
	if (skip_string(&it, end, "currentColor"))
	{
		...
		color->value = 0xFF000000UL;
	} else
	{
		plutovg_color_t value;
		plutovg_int_t length = plutovg_color_parse(&value, it, end - it);
...
static plutosvg_document_t *plutosvg_document_create(float width, float height, plutovg_destroy_func_t destroy_func,
	void *closure)
{
...
static void parse_style(const char *data, plutovg_int_t length, element_t *element, plutosvg_document_t *document)
{
	const char *it = data;
	...
	while (it < end && IS_CSS_STARTNAMECHAR(*it))
	{
		...
		while (it < end && IS_CSS_NAMECHAR(*it))
			++it;
		id = cssattributeid(data, it - data);
...
plutosvg_document_t *plutosvg_document_load_from_data(const char *data, plutovg_int_t length, float width, float height,
	plutovg_destroy_func_t destroy_func, void *closure)
{
	...
	document = plutosvg_document_create(width, height, destroy_func, closure);
	...
	if (it == end && ignoring == 0 && current == NULL && document->root_element)
	{
		...
		intrinsic_width = convert_length(&w, width);
		intrinsic_height = convert_length(&h, height);
...
#define INVALID_RECT(r) PLUTOVG_MAKE_RECT(r, 0, 0, -1, -1)
#define EMPTY_RECT(r) PLUTOVG_MAKE_RECT(r, 0, 0, 0, 0)
...
static void render_state_end(render_state_t *state)
{
	...
	l = MIN(state->parent->extents.x, extents.x);
	t = MIN(state->parent->extents.y, extents.y);
	r = MAX(state->parent->extents.x + state->parent->extents.w, extents.x + extents.w);
	b = MAX(state->parent->extents.y + state->parent->extents.h, extents.y + extents.h);
...
static void convert_color(const color_t *color, plutovg_color_t *c)
{
	plutovg_color_init_argb32(c, color->value);
}

static void resolve_current_color(const render_context_t *context, const element_t *element, plutovg_color_t *c)
{
	for (;;)
	{
		...
		if (element->parent == NULL)
		{
			...
			{
				*c = *context->current_color;
				return;
			}
			PLUTOVG_BLACK_COLOR(c);
			return;
		}
...
static void resolve_color(const render_context_t *context, const element_t *element, const color_t *color, plutovg_color_t *c)
{
	if (color->type == color_type_fixed)
		convert_color(color, c);
	else
		resolve_current_color(context, element, c);
}
...
static void resolve_gradient_stops(const render_context_t *context, const element_t *element,
	gradient_stop_array_t *stops)
{
	...
	while (child && stops->size < MAX_STOPS)
	{
		if (child->id == TAG_STOP)
		{
			...
			color_t stop_color = { color_type_fixed, 0xFF000000UL };
			...
			resolve_color(context, child, &stop_color, &stops->data[stops->size].color);
...
static void resolve_gradient_attributes(const render_context_t *context, const render_state_t *state,
	const gradient_attributes_t *attributes, units_type_t *units,
	plutovg_spread_method_t *spread, plutovg_matrix_t *transform,
	gradient_stop_array_t *stops)
{
...
static bool apply_linear_gradient(render_state_t *state, const render_context_t *context, const element_t *element)
{
	...
	for (i = 0; i < MAX_GRADIENT_DEPTH; ++i)
	{
		const element_t *ref;
		...
		ref = resolve_href(context->document, current);
	...
	if (attributes.x1 == NULL)
		attributes.x1 = element;
	if (attributes.y1 == NULL)
		attributes.y1 = element;
	if (attributes.x2 == NULL)
		attributes.x2 = element;
	if (attributes.y2 == NULL)
	...
	_x1 = resolve_gradient_length(state, &x1, units, 'x');
	_y1 = resolve_gradient_length(state, &y1, units, 'y');
	_x2 = resolve_gradient_length(state, &x2, units, 'x');
	_y2 = resolve_gradient_length(state, &y2, units, 'y');
...
static bool apply_radial_gradient(render_state_t *state, const render_context_t *context, const element_t *element)
{
	...
	if (attributes.base.stops == NULL)
		return false;
	fill_gradient_attributes(element, &attributes.base);
	if (attributes.cx == NULL)
		attributes.cx = element;
	if (attributes.cy == NULL)
		attributes.cy = element;
	if (attributes.r == NULL)
		attributes.r = element;
	...
	_cx = resolve_gradient_length(state, &cx, units, 'x');
	_cy = resolve_gradient_length(state, &cy, units, 'y');
	_r = resolve_gradient_length(state, &r, units, 'o');
	_fx = resolve_gradient_length(state, &fx, units, 'x');
	_fy = resolve_gradient_length(state, &fy, units, 'y');

	plutovg_canvas_set_radial_gradient(context->canvas, _cx, _cy, _r, _fx, _fy, 0.f, spread, stops.data, stops.size,
		&transform);
	return true;
}
...
static bool apply_paint(render_state_t *state, const render_context_t *context, const paint_t *paint)
{
	...
	if (paint->type == paint_type_color)
	{
		...
		resolve_color(context, state->element, &paint->color, &color);
	...
	ref = find_element(context->document, &paint->id);
...
static void draw_shape(const element_t *element, const render_context_t *context, render_state_t *state)
{
	...
	paint_t fill = { paint_type_color, { color_type_fixed, 0xFF000000UL}, { 0, 0 } };
	...
	if (state->mode == render_mode_bounding)
	{
		...
		if (stroke.type == paint_type_none)
			return;
		line_width = resolve_length(state, &stroke_width, 'o');
		cap_limit = line_width / 2.f;

		if (line_cap == PLUTOVG_LINE_CAP_SQUARE)
			cap_limit *= PLUTOVG_SQRT2;
		join_limit = line_width / 2.f;
		...
		delta = MAX(cap_limit, join_limit);
	...
	if (apply_paint(state, context, &stroke))
	{
		...
		for (i = 0; i < (plutovg_int_t)dash_array.size; ++i)
		{
...
static void apply_view_transform(render_state_t *state, float width, float height)
{
	...
	scale_x = width / view_box.w;
	scale_y = height / view_box.h;
...
static void render_symbol(const element_t *element, const render_context_t *context, render_state_t *state, float x,
...
static void render_svg(const element_t *element, const render_context_t *context, render_state_t *state)
{
	...
	_x = resolve_length(state, &x, 'x');
	_y = resolve_length(state, &y, 'y');
	_w = resolve_length(state, &w, 'x');
	_h = resolve_length(state, &h, 'y');
...
static void render_use(const element_t *element, const render_context_t *context, render_state_t *state)
{
	...
	if (is_display_none(element) || has_cycle_reference(state, element))
		return;
	ref = resolve_href(context->document, element);
...
static void render_line(const element_t *element, const render_context_t *context, render_state_t *state)
{
	...
	_x1 = resolve_length(state, &x1, 'x');
	_y1 = resolve_length(state, &y1, 'y');
	_x2 = resolve_length(state, &x2, 'x');
	_y2 = resolve_length(state, &y2, 'y');
...
static void render_ellipse(const element_t *element, const render_context_t *context, render_state_t *state)
{
	...
	_cx = resolve_length(state, &cx, 'x');
	_cy = resolve_length(state, &cy, 'y');
	_rx = resolve_length(state, &rx, 'x');
	_ry = resolve_length(state, &ry, 'y');
...
static void render_rect(const element_t *element, const render_context_t *context, render_state_t *state)
{
	...
	if (!is_length_valid(rx))
		_rx = _ry;
	if (!is_length_valid(ry))
		_ry = _rx;
...
static plutovg_surface_t *load_image(const element_t *element)
{
	...
	if (!skip_string(&it, end, "data:image/png")
		&& !skip_string(&it, end, "data:image/jpg") && !skip_string(&it, end, "data:image/jpeg"))
	{
		return NULL;
	}
...
static void draw_image(const element_t *element, const render_context_t *context, render_state_t *state, float x,
	float y, float width, float height)
{
	float scale_x, scale_y;
	...
	if (state->mode == render_mode_bounding)
		return;
	image = load_image(element);

	if (image == NULL)
		return;
	image_width = plutovg_surface_get_width(image);
	image_height = plutovg_surface_get_height(image);
	...
	scale_x = dst_rect.w / src_rect.w;
	scale_y = dst_rect.h / src_rect.h;
	matrix.a = scale_x;
	matrix.b = 0;
	matrix.c = 0;
	matrix.d = scale_y;
	matrix.e = -src_rect.x * scale_x;
	matrix.f = -src_rect.y * scale_y;
...
bool plutosvg_document_render(const plutosvg_document_t *document, const char *id, plutovg_canvas_t *canvas,
	const plutovg_color_t *current_color, plutosvg_palette_func_t palette_func, void *closure)
{
	...
	context.palette_func = palette_func;
	...
	} else
	{
		...
		element = find_element(document, &name);
		if (element == NULL)
			return false;
		state.element = element;
	}
...
plutovg_surface_t *plutosvg_document_render_to_surface(const plutosvg_document_t *document, const char *id, plutovg_int_t width,
	plutovg_int_t height, const plutovg_color_t *current_color,
	plutosvg_palette_func_t palette_func, void *closure)
{
	...
	extents.x = 0;
	extents.y = 0;
	extents.w = document->width;
	extents.h = document->height;
	...
	if (width <= 0 && height <= 0)
	{
		width = (plutovg_int_t) (ceilf(extents.w));
		height = (plutovg_int_t) (ceilf(extents.h));
	} else if (width > 0 && height <= 0)
	{
		height = (plutovg_int_t) (ceilf(width * extents.h / extents.w));
	} else if (height > 0 && width <= 0)
	{
		width = (plutovg_int_t) (ceilf(height * extents.w / extents.h));
	}
...
bool plutosvg_document_extents(const plutosvg_document_t *document, const char *id, plutovg_rect_t *extents)
{
	...
	context.document = document;
	context.canvas = NULL;
	context.current_color = NULL;
	context.palette_func = 0;
...
{
	ft_svg_state_t *state = malloc(sizeof(*state));
...
static FT_Error ft_svg_render(FT_GlyphSlot ft_slot, FT_Pointer *ft_state)
{
	...
	plutovg_surface_t *surface;
	plutovg_canvas_t *canvas;
	FT_SVG_Document ft_document;
	FT_UShort start_glyph_id, end_glyph_id;

	if (state == NULL || state->document == NULL)
		return FT_Err_Invalid_SVG_Document;
	surface = plutovg_surface_create_for_data(ft_slot->bitmap.buffer, ft_slot->bitmap.width, ft_slot->bitmap.rows, ft_slot->bitmap.pitch);
	if (surface != NULL)
	{
		canvas = plutovg_canvas_create(surface);
		...
		if (canvas != NULL)
		{
			ft_document = (FT_SVG_Document) ft_slot->other;
			start_glyph_id = ft_document->start_glyph_id;
			end_glyph_id = ft_document->end_glyph_id;
...
static plutosvg_document_t *ft_svg_document_load(ft_svg_state_t *state, const FT_Byte *data, FT_ULong length, FT_UShort units_per_EM)
{
	...
	document = plutosvg_document_load_from_data((const char *) data, length, units_per_EM, units_per_EM, NULL, NULL);
	if (document == NULL)
		return NULL;
...
static FT_Error ft_svg_preset_slot(FT_GlyphSlot ft_slot, FT_Bool ft_cache, FT_Pointer *ft_state)
{
	...
	ft_document = (FT_SVG_Document) ft_slot->other;
	ft_metrics = ft_document->metrics;

	start_glyph_id = ft_document->start_glyph_id;
	end_glyph_id = ft_document->end_glyph_id;

	document = ft_svg_document_load(state, ft_document->svg_document, ft_document->svg_document_length, ft_document->units_per_EM);
	if (document == NULL)
	{
		return FT_Err_Invalid_SVG_Document;
	}

	transform.a = (float) ft_document->transform.xx / ((int32_t)1 << 16);
	transform.b = -(float) ft_document->transform.xy / ((int32_t)1 << 16);
	transform.c = -(float) ft_document->transform.yx / ((int32_t)1 << 16);
	transform.d = (float) ft_document->transform.yy / ((int32_t)1 << 16);
	transform.e = (float) ft_document->delta.x / 64 * document->width / ft_metrics.x_ppem;
	transform.f = -(float) ft_document->delta.y / 64 * document->height / ft_metrics.y_ppem;

	x_svg_to_out = ft_metrics.x_ppem / document->width;
	y_svg_to_out = ft_metrics.y_ppem / document->height;
	...
	metrics_width = extents.w;
	metrics_height = extents.h;

	horiBearingX = extents.x;
	horiBearingY = -extents.y;

	vertBearingX = ft_slot->metrics.horiBearingX / 64.f - ft_slot->metrics.horiAdvance / 64.f / 2;
	vertBearingY = (ft_slot->metrics.vertAdvance / 64.f - ft_slot->metrics.height / 64.f) / 2;
...
{
	static SVG_RendererHooks svg_hooks = {
		ft_svg_init,
		ft_svg_free,
		ft_svg_render,
		ft_svg_preset_slot
