...
plutovg_int_t plutovg_version(void)
{
...
static void plutovg_state_reset(plutovg_state_t *state)
{
	...
	state->paint = NULL;
	PLUTOVG_BLACK_COLOR(&state->color);
...
plutovg_canvas_t *plutovg_canvas_create(plutovg_surface_t *surface)
{
	...
	PLUTOVG_MAKE_RECT(&canvas->clip_rect, 0, 0, surface->width, surface->height);
...
plutovg_int_t plutovg_canvas_get_reference_count(const plutovg_canvas_t *canvas)
{
	if (canvas == NULL)
		return 0;
...
void plutovg_canvas_set_linear_gradient(plutovg_canvas_t *canvas, float x1, float y1, float x2, float y2,
	plutovg_spread_method_t spread, const plutovg_gradient_stop_t *stops,
	plutovg_int_t nstops, const plutovg_matrix_t *matrix)
{
...
void plutovg_canvas_set_radial_gradient(plutovg_canvas_t *canvas, float cx, float cy, float cr, float fx, float fy,
	float fr, plutovg_spread_method_t spread, const plutovg_gradient_stop_t *stops,
	plutovg_int_t nstops, const plutovg_matrix_t *matrix)
{
	plutovg_paint_t *paint =
		plutovg_paint_create_radial_gradient(cx, cy, cr, fx, fy, fr, spread, stops, nstops, matrix);
	plutovg_canvas_set_paint(canvas, paint);
	plutovg_paint_destroy(paint);
}

void plutovg_canvas_set_texture(plutovg_canvas_t *canvas, plutovg_surface_t *surface, plutovg_texture_type_t type,
	float opacity, const plutovg_matrix_t *matrix)
{
...
void plutovg_canvas_set_dash(plutovg_canvas_t *canvas, float offset, const float *dashes, plutovg_int_t ndashes)
{
...
void plutovg_canvas_set_dash_array(plutovg_canvas_t *canvas, const float *dashes, plutovg_int_t ndashes)
{
...
plutovg_int_t plutovg_canvas_get_dash_array(const plutovg_canvas_t *canvas, const float **dashes)
{
	if (dashes)
...
void plutovg_canvas_set_matrix(plutovg_canvas_t *canvas, const plutovg_matrix_t *matrix)
{
	if (matrix)
		...
		PLUTOVG_IDENTITY_MATRIX(&canvas->state->matrix);
}
...
void plutovg_canvas_arc_to(plutovg_canvas_t *canvas, float rx, float ry, float angle, bool large_arc_flag,
...
void plutovg_canvas_stroke_extents(const plutovg_canvas_t *canvas, plutovg_rect_t *extents)
{
	...
	join_limit = stroke->style.width / 2.f;
	...
	delta = plutovg_max(cap_limit, join_limit);
...
float plutovg_canvas_add_text(plutovg_canvas_t *canvas, const void *text, plutovg_int_t length, plutovg_text_encoding_t encoding,
	float x, float y)
{
	plutovg_state_t *state = canvas->state;
...
float plutovg_canvas_fill_text(plutovg_canvas_t *canvas, const void *text, plutovg_int_t length, plutovg_text_encoding_t encoding,
	float x, float y)
{
	float advance_width;

	plutovg_canvas_new_path(canvas);
	advance_width = plutovg_canvas_add_text(canvas, text, length, encoding, x, y);

	plutovg_canvas_fill(canvas);
	return advance_width;
}

float plutovg_canvas_stroke_text(plutovg_canvas_t *canvas, const void *text, plutovg_int_t length,
	plutovg_text_encoding_t encoding, float x, float y)
{
	float advance_width;
...
float plutovg_canvas_clip_text(plutovg_canvas_t *canvas, const void *text, plutovg_int_t length, plutovg_text_encoding_t encoding,
	float x, float y)
{
	float advance_width;
...
void plutovg_canvas_font_metrics(const plutovg_canvas_t *canvas, float *ascent, float *descent, float *line_gap,
...
void plutovg_canvas_glyph_metrics(plutovg_canvas_t *canvas, plutovg_codepoint_t codepoint, float *advance_width,
	float *left_side_bearing, plutovg_rect_t *extents)
{
	...
	if (state->font_face && state->font_size > 0.f)
	{
		plutovg_font_face_get_glyph_metrics(state->font_face, state->font_size, codepoint, advance_width,
			left_side_bearing, extents);
		return;
	}
...
float plutovg_canvas_text_extents(plutovg_canvas_t *canvas, const void *text, plutovg_int_t length,
