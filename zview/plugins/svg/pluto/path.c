...
plutovg_int_t plutovg_path_get_reference_count(const plutovg_path_t *path)
{
	if (path)
...
plutovg_int_t plutovg_path_get_elements(const plutovg_path_t *path, const plutovg_path_element_t **elements)
{
	if (elements)
...
static plutovg_path_element_t *plutovg_path_add_command(plutovg_path_t *path, plutovg_path_command_t command,
	plutovg_int_t npoints)
{
	const plutovg_int_t length = npoints + 1;
	plutovg_path_element_t *elements;

	plutovg_array_ensure(path->elements, length);
	elements = path->elements.data + path->elements.size;
...
void plutovg_path_line_to(plutovg_path_t *path, float x, float y)
{
	...
	elements = plutovg_path_add_command(path, PLUTOVG_PATH_COMMAND_LINE_TO, 1);
...
void plutovg_path_quad_to(plutovg_path_t *path, float x1, float y1, float x2, float y2)
{
	...
	cp1x = 2.f / 3.f * x1 + 1.f / 3.f * current_x;
	cp1y = 2.f / 3.f * y1 + 1.f / 3.f * current_y;
	cp2x = 2.f / 3.f * x1 + 1.f / 3.f * x2;
	cp2y = 2.f / 3.f * y1 + 1.f / 3.f * y2;
...
void plutovg_path_cubic_to(plutovg_path_t *path, float x1, float y1, float x2, float y2, float x3, float y3)
{
	...
	if (path->elements.size == 0)
		plutovg_path_move_to(path, 0, 0);
	elements = plutovg_path_add_command(path, PLUTOVG_PATH_COMMAND_CUBIC_TO, 3);
...
void plutovg_path_arc_to(plutovg_path_t *path, float rx, float ry, float angle, bool large_arc_flag, bool sweep_flag,
	float x, float y)
{
	float current_x, current_y;
	plutovg_int_t segments;
	float dx, dy;
	float rxrx, ryry, dxdx, dydy, radius;
	float x1, y1;
	float x2, y2;
	float dx1, dy1, d, scale_sq, scale;
	float cx1, cy1;
	float th1, th2, th_arc;
	...
	th1 = atan2f(y1 - cy1, x1 - cx1);
	th2 = atan2f(y2 - cy1, x2 - cx1);
	th_arc = th2 - th1;
	...
	segments = (plutovg_int_t) (ceilf(fabsf(th_arc / (PLUTOVG_HALF_PI + 0.001f))));
...
void plutovg_path_close(plutovg_path_t *path)
{
	...
	if (path->elements.size == 0)
		return;
	elements = plutovg_path_add_command(path, PLUTOVG_PATH_COMMAND_CLOSE, 1);
...
void plutovg_path_reserve(plutovg_path_t *path, plutovg_int_t count)
{
...
void plutovg_path_add_round_rect(plutovg_path_t *path, float x, float y, float w, float h, float rx, float ry)
{
	...
	right = x + w;
	bottom = y + h;
	cpx = rx * PLUTOVG_KAPPA;
	cpy = ry * PLUTOVG_KAPPA;
...
void plutovg_path_add_arc(plutovg_path_t *path, float cx, float cy, float r, float a0, float a1, bool ccw)
{
	...
	seg_n = (plutovg_int_t) (ceilf(fabsf(da) / PLUTOVG_HALF_PI));
	...
	seg_a = da / seg_n;
	d = (seg_a / PLUTOVG_HALF_PI) * PLUTOVG_KAPPA * r;
	dx = -sinf(a) * d;
	dy = cosf(a) * d;
...
void plutovg_path_transform(plutovg_path_t *path, const plutovg_matrix_t *matrix)
{
	...
	for (i = 0; i < path->elements.size; i += elements[i].header.length)
	{
	...
	while (plutovg_path_iterator_has_next(&it))
	{
		...
		case PLUTOVG_PATH_COMMAND_CUBIC_TO:
			...
			while (b >= beziers)
			{
				...
				if (l > 1.f)
				{
					d = fabsf((x4x1) * (b->y1 - b->y2) - (y4y1) * (b->x1 - b->x2)) + fabsf((x4x1) * (b->y1 - b->y3) -
																						   (y4y1) * (b->x1 - b->x3));
				} else
				{
...
{
	const float *dashes;
	plutovg_int_t ndashes;
	float start_phase;
	float phase;
	plutovg_int_t start_index;
	plutovg_int_t index;
	bool start_toggle;
	bool toggle;
...
static void dash_traverse_func(void *closure, plutovg_path_command_t command, const plutovg_point_t *points, plutovg_int_t npoints)
{
	dasher_t *dasher = (dasher_t *) (closure);
	plutovg_point_t p0, p1;
	float dx, dy;
	float dist0, dist1;
	...
	p0 = dasher->current_point;
	p1 = points[0];
	dx = p1.x - p0.x;
	dy = p1.y - p0.y;
	dist0 = sqrtf(dx * dx + dy * dy);
	dist1 = 0.f;
...
void plutovg_path_traverse_dashed(const plutovg_path_t *path, float offset, const float *dashes, plutovg_int_t ndashes,
...
static void clone_traverse_func(void *closure, plutovg_path_command_t command, const plutovg_point_t *points, plutovg_int_t npoints)
{
...
plutovg_path_t *plutovg_path_clone_dashed(const plutovg_path_t *path, float offset, const float *dashes, plutovg_int_t ndashes)
{
	plutovg_path_t *clone = plutovg_path_create();
...
static void extents_traverse_func(void *closure, plutovg_path_command_t command, const plutovg_point_t *points, plutovg_int_t npoints)
{
	...
	for (i = 0; i < npoints; ++i)
	{
		...
		if (command != PLUTOVG_PATH_COMMAND_MOVE_TO)
			calculator->length +=
				hypotf(points[i].x - calculator->current_point.x, points[i].y - calculator->current_point.y);
...
static inline bool parse_path_coordinates(const char **begin, const char *end, float values[6], plutovg_int_t offset, plutovg_int_t count)
{
	plutovg_int_t i;
...
bool plutovg_path_parse(plutovg_path_t *path, const char *data, plutovg_int_t length)
{
	const char *it;
	const char *end;
	float values[6];
	...
	while (it < end)
	{
		...
		} else if (command == 'A' || command == 'a')
		{
			if (!parse_path_coordinates(&it, end, values, 0, 3)
				...
				|| !parse_arc_flag(&it, end, &flags[1]) || !parse_path_coordinates(&it, end, values, 3, 2))
			{
				return false;
			}
